...
// RGBA floats in [0,1] - the same representation and component order
// ImGui::ColorEdit3/4 and ImGui::ColorConvertFloat4ToU32/U32ToFloat4
// already expect, so no bit-unpacking is ever needed. Colors used to be
// stored as packed RRGGBBAA ints instead, which forced every call site to
// hand-roll its own conversion.
//
// idleColor/customColor (CyclicGroup / CyclicGroup::Slot in events.h) are
// NOT part of this - they're already native ImU32, fed straight into
// ImDrawList calls. They still get converted for editing (ColorEdit4 wants
// floats), just via the plain ColorFloat4/ColorU32 pair below rather than
// a bespoke wrapper.
//--------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------
// Straight 4-float copy of a SETTING_ARRAY color (stored as plain float[4]
// so the generic array-setting machinery in settings.h/.cpp doesn't need a
// color-specific macro) into ImVec4, for call sites that want member/
// struct syntax (c.w *= alpha, ImGui::TextColored, etc.). ToImVec4Opaque
// forces alpha to 1.0, for the handful of call sites (Subscriptions
// window's Active/Soon text, the weekly-tracked '*' marker) that feed
// straight into ImGui::TextColored, which has no separate opacity control.
//--------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------
// Plain conversion to a drawable ImU32, no adjustment - e.g. ColorSet::pri()
// in events.h.
//--------------------------------------------------------------------------------
...
// idleColor/customColor (events.h) are stored as native ImU32 rather than
// one of this addon's own RGBA-float settings, so there's no ToImVec4/
// ColorU32 float-array pair for them - this is the one place a stored
// color needs converting FROM ImU32.
//--------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------
// Scales R/G/B by factor, alpha unchanged - derives a color's secondary/
// tertiary shade (ColorSet::sec()/ter() in events.h).
//--------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------
// Multiplies alpha by alphaMul, R/G/B unchanged - the notification toast's
// fade-in/out, the subscription bar's dropping dots/segments, and
// ThemeColorU32 below all animate opacity this same way.
//--------------------------------------------------------------------------------
