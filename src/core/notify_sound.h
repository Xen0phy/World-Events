...
// ScanNotificationSoundFiles / GetNotificationSoundFilenames
//--------------------------------------------------------------------------------
// Sorted, ".wav"-only filenames (no path) from "<addon dir>/sounds" - see
// PlayNotificationSound for why only .wav. Scan() re-scans to pick up newly-added
// files (no filesystem-watching), creating the directory first if needed. Get()
// lazily scans on first call.
//--------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------
// Plays "<addon dir>/sounds/<filename>" via winmm's PlaySound (SND_FILENAME |
// SND_ASYNC) - non-blocking, safe from the render thread (both current call
// sites: the options panel's "Test" button, and subscriptions_notification.cpp's
// per-candidate sound opt-in). A no-op if filename is empty or the file no longer
// exists on disk.
//--------------------------------------------------------------------------------
