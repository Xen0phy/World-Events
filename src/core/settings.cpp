...
// parse<T>
//--------------------------------------------------------------------------------
// One specialization per settings_table.h Type. Throws on a malformed
// value (see LoadSettings below, which catches per-field).
//--------------------------------------------------------------------------------
...
// Two shapes accepted: CURRENT "r,g,b,a" (comma-joined floats in [0,1],
// written by SaveSettings above - the normal case), or LEGACY a single
// packed-RRGGBBAA integer, only ever seen on the first load of a
// settings.ini written by an older build.
//
// Sets *wasLegacy = true only for the legacy case, so LoadSettings can
// trigger exactly one SaveSettings() rewrite after a load that actually
// touched the old format - without this, a legacy file would silently
// keep re-migrating in memory every run without ever updating on disk.
//--------------------------------------------------------------------------------
