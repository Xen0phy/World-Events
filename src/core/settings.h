...
// settings.h
//--------------------------------------------------------------------------------
// Declares one extern global per entry in settings_table.h, plus the load/save
// functions that persist them to settings.ini in the addon directory.
//
// Definitions (actual storage) live in settings.cpp - exactly one translation
// unit defines these globals.
//--------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------
// X-macros expanding settings_table.h into one extern declaration per setting
// (see settings.cpp for the storage/load/save side). SETTING_ARRAY uses a plain
...
// try/catch blocks. LoadSettings silently ignores unknown keys/sections, so a
// settings.ini written by an older build loads cleanly even if new keys were
// added since - and a missing file simply means every global keeps the compiled-
// in default from settings_table.h.
//--------------------------------------------------------------------------------
