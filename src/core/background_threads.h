...
// (Nexus) FreeLibrary()s the module - which is undefined behavior / a crash risk,
// since a detached std::thread has no built-in way to be waited on.
//
...
//   }).detach();
//
// If a step can block INSIDE a single OS call for a long time with no natural
// checkpoint (e.g. a synchronous WinHTTP request already in flight),
// IsShuttingDown() checkpoints alone can't interrupt it - register a shutdown
// hook (RegisterShutdownHook below) that forces that specific call to return
// early instead.
//
// Usage from AddonUnload, BEFORE touching any state a background thread might
// also touch:
//
...
// (returning, so their BackgroundThreadGuard destructs) once it goes true, rather
// than doing any more work whose results will just be discarded.
//--------------------------------------------------------------------------------
bool IsShuttingDown();
...
// timeoutMs elapses.
//
// Kept short: this is a last-resort backstop for a thread that didn't respond to
// the hooks/checkpoints above, not the primary mechanism - a long value here
// would stall the whole game's shutdown if the host calls AddonUnload as part of
// an orderly exit. A hard process kill bypasses this function entirely and
// terminates every thread at once, so this wait only matters for "this DLL
// unloads while the game keeps running."
//--------------------------------------------------------------------------------
