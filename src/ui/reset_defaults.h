...
// reset_defaults.h
//--------------------------------------------------------------------------------
// "Default" button of the settings window's Events tab (options_events.cpp).
// Deletes events.json and rebuilds every list it holds - Basic Events, Cyclic
// Groups, categories, subscriptions/watchlist, and done-today markers - back to
// the compiled-in roster, wiping any user edits/customization along with it.
// Destructive and irreversible in-session, so a confirm popup sits between the
// button and ResetAllDataToDefaults() (addon.h) actually running.
//--------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------
// Call DrawResetToDefaultsButton() from DrawOptionsEvents to add the "Default"
// button; it opens the confirm popup managed here. Call
// DrawResetToDefaultsPopup() every frame from the same place and ID scope (the
// popup ID is resolved against the current ID stack), regardless of whether the
// popup is currently open, so it renders while open.
//--------------------------------------------------------------------------------
