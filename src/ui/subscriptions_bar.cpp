...
//
// Layered bottom-up:
//  - CollectVisibleSegments/AssignLanes turn the shared resolved-subscriptions
//    cache into LineSegments placed in local pixel space, with overlapping items
//    pushed to hidden lanes (dot markers only).
//  - PackStackRows/pillStackY lay out the vertical stack of currently
//    hovered/dropping segments, detaching into a pill when a segment would
//    otherwise cover an "unsafe" GW2-UI zone or share x-range with something
//    already occupying row 0.
//  - PathFlatBlockShoulders/FillFlatBlockShoulders/PathRoundedRect are the actual
//    geometry: a flat-top block with curved shoulders that necks in and detaches
//    into a stadium-cap pill as DropState::amount eases from 0 to 1 on hover
//    (with a click-hold override, see kClickHoldSeconds).
...
#include "addon.h" //. SubsBarDataTimer/SubsBarDrawTimer live here
...
//_ The line always spans exactly this much time from "now"; local x 0..W along the strip maps linearly to 0..kWindowSeconds
...
static std::vector<LineSegment> CollectVisibleSegments(time_t now, float stripWidth)
{
    ...
    //_ shared cache - RefreshSubscriptionsCache already ran once at the top of RenderSubscriptionsBar
    ...
    {
        //_ "Already done today" skip (API-confirmed or manually marked) - see ResolvedSubscription::doneToday.
...
//_ Pill-detach phases, as fractions of DropState::amount: neck-in at kPinchStart, detach at kPinchEnd, settled at kDetachEnd.
...
static void PathFlatBlockShoulders(ImDrawList* dl, float start, float end, float baselineY, float blockH, float tw, float depth, float dropDir = 1.0f)
{
    ...
    //_ fx/fy fractions per point: fx is 0 at the baseline corner, 1 at the flat-top corner; fy is 0 at the baseline, 1 at the flat top.
...
//--------------------------------------------------------------------------------
static void FillFlatBlockShoulders(
    ImDrawList* dl,
    float       start,
    float       end,
    float       baselineY,
    ...
    float       dropDir = 1.0f)
{
    ...
    //_ One combined vertex buffer (rise, fall, two inner-top corners); every triangle indexes into it, including at the rect/cap seams, so shared edges reuse the same vertex instead of duplicating it.
...
static void PathRoundedRect(ImDrawList* dl, ImVec2 p0, ImVec2 p1, float rounding)
{
    ...
    //_ Starts at "kPi * 2.0f", not the equivalent 0.0f, so cosf/sinf match top-right's end point bit-for-bit and avoid a few-ULP seam gap.
...
void RenderSubscriptionsBar()
{
    ...
    //_ Manual lists being empty doesn't mean nothing renders - weekly auto-tracked segments (WeeklyAutoTrackEnabled, subscriptions_cache.cpp) populate segs independently; segs.empty() below is the real check.
    ...
    std::vector<LineSegment> segs;
    {
        //_ Scoped to just data gathering/resolution (RefreshSubscriptionsCache is a no-op most frames). Split from SubsBarDrawTimer below so the debug line can separate re-deriving cost from rendering cost.
    ...
    //_ Everything below: dot layout, hover detection, and the actual ImGui draw-list calls - see g_AvgSubsBarDrawMs's comment in addon.h.
    SubsBarDrawTimer drawTimer;
    //_ Minimal mode hides the per-segment colored baseline, so every event needs its own dot instead of just the hidden lane>0 ones.
    ...
    //_ Flips every depth offset so blocks/pills/dots grow up off the bottom edge when bottom-anchored, instead of down off the top edge.
    ...
    //_ Normal mode offsets dots below the colored line; minimal mode has no colored line, so its dots sit directly on the baseline instead.
    ...
    //_ Nudged draw-x per dot, so same-tick dots spread into a small cluster instead of overlapping.
    ...
    //_ Segments under the mouse, via two paths: (1) a lane-0 segment's own x-range within a shared hover band, so the mouse can travel onto an open block without losing hover; (2) a dot marker (both feed hoveredIndices).
    ...
    //_ Shared sustain-band far edge: baseline by default, pushed out along kDropDir to cover whatever's currently deepest (estimated from last frame's state - one row per open segment, an upper bound on height).
    ...
    if (mouseValid)
    {
        if (mouse.y >= kBaselineY - kHoverBand && mouse.y <= kBaselineY + kHoverBand)
        {
            for (int i = 0; i < (int)segs.size(); i++)
            {
                //_ lane>0 segments have no line of their own to hover
                if (segs[i].lane != 0) continue;
                if (mouse.x >= segs[i].startX && mouse.x < segs[i].endX) hoveredIndices.push_back(i);
            }
        }
        //_ Sustain path: segments already mid-drop, tested against the shared band (near edge here, far edge sustainBottom, padded by kSustainPadX) so a dot-triggered open survives leaving the hit-circle.
    ...
    //_ Track raw hover duration, then ease every segment's drop amount toward a delay-gated target.
    ...
    for (int i = 0; i < (int)segs.size(); i++)
    {
        ...
        //_ clickHoldSeconds forces hovered regardless of real mouse position; forcePastDelay only applies if the click had already cleared this segment's own hover delay through real dwell time (see below).
        ...
        //_ Drop targets 1.0 only once hover clears the configured delay; losing hover always targets 0 immediately, with no delay back down.
    ...
    //_ Drop stale keys (segment no longer visible this frame) so the map doesn't grow unbounded.
    ...
    //_ Dot markers: white, red for an active-and-incomplete weekly Wizard's Vault target (see weekly_vault.h/.cpp for isWeekly), fading out as their own segment drops in.
    for (size_t d = 0; d < dots.size(); d++)
    {
    ...
    //_ Top-of-block Y for each hovered segment, filled in by the row-packing below; drops always grow from the baseline, moving toward unsafe-zone clearance only once fully detached into a pill.
    std::unordered_map<std::string, float> stackTopY;

    //_ Edge-safe drop bounds, precomputed per segment so row-packing, drawing, and click hit-testing all agree.
...

    {
        ...
        //_ Row height is NOT scaled by live eased depth (an open row and a still-easing row below must never overlap), so every occupied row reserves full kMaxDropPx unconditionally.
    ...
    //_ Colored baseline overlay is lane-0 only; the block/pill drop applies to any lane.
    for (int i = 0; i < (int)segs.size(); i++)
    {
        const LineSegment& seg = segs[i];
        float depth = s_dropStates[seg.key].amount;

        //_ The resting baseline line always uses the segment's true x-range; only the dropped block/pill below uses the edge-safe widened bounds.
        float x0 = seg.startX;
        float x1 = seg.endX;
        float segEnd = x1;
        ...
        //_ Nexus's own WindowBg (ThemeColorU32, shared with subscriptions_notification.cpp), not segColor - so the pop-out reads as UI chrome, not a solid tint of the event's own color.
        ...
        {
            //_ Colored resting line - only lane-0 segments get one; overlap is signaled by dots instead.
            dl->AddLine(ImVec2(x0, kBaselineY), ImVec2(segEnd, kBaselineY), segColor, kLineThick + 1.0f);
        }

        if (depth > 0.002f)
        {
            //_ Stacked target Y: this segment's reserved slot if hovered, otherwise the baseline (mid-ease-out, no resting Y of its own).
            float topY = kBaselineY;
            auto stackIt = stackTopY.find(seg.key);
            if (stackIt != stackTopY.end()) topY = stackIt->second;

            //_ Edge-safe drop bounds (only affect the drop shape, not the baseline line above).
            ...
            //_ A segment detaches into a pill if it overlaps an unsafe zone, or if it's not in row 0 of the current stack.
            ...
            //_ Phase split (see kPinchStart/kPinchEnd/kDetachEnd): grow -> neck-in -> detach into a locked-width pill -> settle.
            ...
            {
                //_ Attached FlatBlockDepthAt silhouette; shoulders neck in via PillPinchFactor once past kPinchStart.
                float tw = kTransitionWidth;

                if (pinchT <= 0.0f)
                {
                    //_ Common case: real bezier-curve shoulders via PathFlatBlockShoulders/FillFlatBlockShoulders.
                ...
                {
                    //_ Neck-in sub-phase: PillPinchFactor's waist reshapes every frame, so this samples FlatBlockDepthAt point- by-point instead of using the fixed bezier path.
            ...
            {
                //_ Detached: a rounded rect locked to the segment's own on-bar width, corner radius eased via pillRx. Assumes p0.y < p1.y; order by min/max since pillY can be either edge.
    ...
    //_ Click-on-the-thin-line: pop out every segment overlapping mouse.x. Both click blocks below use an invisible window + InvisibleButton, since a plain IsMouseClicked() doesn't fire on this draw list.
    {
        ...
        //_ Default WindowPadding would offset the clickable rect from the actual line.
        ImGui::PushStyleVar(ImGuiStyleVar_WindowPadding, ImVec2(0, 0));
        //_ NoBringToFrontOnFocus is omitted: letting a click bring this window to front keeps its z-order self-healing.
        ...
        if (lineClicked)
        {
            if (mouse.y >= kBaselineY - kHoverBand && mouse.y <= kBaselineY + kHoverBand)
            {
                ...
                //_ Require the clicked segment to have already cleared the hover delay through real dwell time before a click does anything.
                ...
                if (clickedPastDelay)
                {
                    ...
                    //_ Trigger every segment whose own x-range contains mouse.x (not just the whole clicked span), lane>0 included.
    ...
    //_ Click hit-testing: click an already-dropped block or pill to copy its waypoint. Left-click copies the code (only segments that have finished dropping are eligible); right-click marks it done for today.
    for (int idx : hoveredIndices)
    {
        const LineSegment& s = segs[idx];
        float depth = s_dropStates[s.key].amount;
        ...
        //_ Edge-safe bounds from dropBoundsByKey, so the click window matches what was actually drawn/row-packed.
        float dropX0 = x0, dropX1 = segEnd;
        {
            auto boundsIt = dropBoundsByKey.find(s.key);
            if (boundsIt != dropBoundsByKey.end())
        ...
        //_ topY is the block's origin edge (pop-out side); the window needs the true top-left corner regardless of anchor.
