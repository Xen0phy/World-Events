...
// objective's real title from /v2/account/wizardsvault/weekly - usually
// just the objective's own "Events in <region>" clauses (see
// weekly_vault.h), not the addon's own internal names. A keyword that
// stops matching just means that mapping silently never lights up at
// runtime - nothing will warn you, so if a normally-rotating objective
// stops tracking, check here first. check_weekly_vault.py (build time)
// catches the OTHER half of this table going stale - a targets entry that
// no longer resolves against events_cyclic.cpp - but can't verify
...
// Slot::name (events_cyclic.cpp) exactly - the addon's own internal names,
// not necessarily the in-game event's displayed name. Point at the ONE (or
// few) slot(s) that actually complete the meta, not the whole group - most
// groups have build-up slots that don't finish anything on their own.
//--------------------------------------------------------------------------------
...
// gw2_api.cpp's own AsciiLower). Every boss name/titleKeyword used here is
// plain ASCII, so no locale/UTF-8 handling is needed. Not shared with
// gw2_api.cpp's copy since that one is file-static there - too small to be
// worth a shared header just for this.
//--------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------
// See weekly_vault.h for the full contract. Searches
// GetLiveWeeklyObjectives() (gw2_api.h) for a live title containing the
// event's own name.
//--------------------------------------------------------------------------------
...
//--------------------------------------------------------------------------------
// See weekly_vault.h for the full contract. Searches
// g_CyclicWeeklyObjectives for a mapping listing this slot, then
// GetLiveWeeklyObjectives() for a live title matching ALL of that
// mapping's titleKeywords.
//--------------------------------------------------------------------------------
